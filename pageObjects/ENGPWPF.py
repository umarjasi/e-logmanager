...
class ENGPWPF():
    lnk_qc_xpath = "//span[normalize-space()='Quality Control']"
    lnk_eng_xpath = "//span[normalize-space()='Engineering']"
    lnk_pwpf_xpath = "//span[contains(text(),'Purified Water Plant Filter Cleaning and Filter Re')]"
    btn_adnw_id = "btnAddnew"
    dro_tof_id = "WaterPlantFilterData_FilterType"
    dro_cs_id = "WaterPlantFilterData_CleaningStatus"
    txt_fr_id = "WaterPlantFilterData_ReplacedBy"
    drp_fn_id = "WaterPlantFilterData_FloorName"
    txt_rm_id = "WaterPlantFilterData_InitiateRemark"
    ...
    def ClickQC(self):
        self.driver.find_element(By.XPATH, self.lnk_qc_xpath).click()

    def ClickEng(self):
        self.driver.find_element(By.XPATH, self.lnk_eng_xpath).click()

    def ClickPWPF(self):
        self.driver.find_element(By.XPATH, self.lnk_pwpf_xpath).click()

    def ClickAddnew(self):
        self.driver.find_element(By.ID, self.btn_adnw_id).click()

    def TyofFilter(self):
        drop_down_field = self.driver.find_element(By.ID, self.dro_tof_id)
        select = Select(drop_down_field)
        select.select_by_index(2)

    def CleaningStatus(self):
        drop_down_field = self.driver.find_element(By.ID, self.dro_cs_id)
        select = Select(drop_down_field)
        select.select_by_visible_text("Not Cleaned")

    def FilterReplaby(self,FR):
        self.driver.find_element(By.ID, self.txt_fr_id).send_keys(FR)

    def Floor(self):
        drop_down_field = self.driver.find_element(By.ID,self. drp_fn_id)
        select = Select(drop_down_field)
        select.select_by_index(1)

    def Remarks(self, RE):
        self.driver.find_element(By.ID,self.txt_rm_id).send_keys(RE)
