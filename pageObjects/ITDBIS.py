...
class ITDBIS():
     ...
     lnk_ot_xpath = "//span[normalize-space()='Data Backup In IT Server']"
     drp_ssb_xpath = "//select[@id='DataBackupITServerData_Block']"
     txtbox_sfn_id = "DataBackupITServerData_FolederName"
     txtbox_rmarks_id = "DataBackupITServerData_Remarks"
     btn_subm_id = "btnStart"
     txtbox_esus_name = "txtUserName"
     txtbox_espa_name = "txtPassword"
     btn_sv_id = "btnESave"
     txtbox_lof_id = "DataBackupITServerData_ListFilesStored"
     txtbox_itf_id = "DataBackupITServerData_IntegrityFileSize"
     txtbox_bacda_name = "DataBackupITServerData.BackupCopiedServer"
     cal_bacda_xpath = "//td[@class='day'][normalize-space()='10']"
     ...
     def SetSiteBlock(self):
          rm_ele = self.driver.find_element(By.XPATH, self.drp_ssb_xpath)
          rmd = Select(rm_ele)
          rmd.select_by_index(2)

     def SetFoldername(self, SFN):
          self.driver.find_element(By.ID, self.txtbox_sfn_id).send_keys(SFN)

     def setremarks(self, sr):
          self.driver.find_element(By.ID, self.txtbox_rmarks_id).send_keys(sr)
     ...
     def scrolldown(self):
          ...
          init = self.driver.find_element(By.XPATH,"/html[1]/body[1]/div[1]/div[1]/div[1]/div[2]/nav[1]/ul[1]/li[2]/div[1]/ul[1]/li[6]/a[1]/span[1]")
          self.driver.execute_script("arguments[0].scrollIntoView();", init)
     ...
     def SetLstoffiles(self, LOF):
          self.driver.find_element(By.ID, self.txtbox_lof_id).send_keys(LOF)

     def SetIntegTest(self, ITF):
          self.driver.find_element(By.ID, self.txtbox_itf_id).send_keys(ITF)

     def Backupdate(self):
          self.driver.find_element(By.NAME, self.txtbox_bacda_name).click()

     def calendarBackupdate(self):
          self.driver.find_element(By.XPATH, self.cal_bacda_xpath).click()
